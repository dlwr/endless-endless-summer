...
2. [最新版をインストール](https://github.com/dlwr/endless-endless-summer/releases/latest/download/endless-endless-summer.user.js)(マネージャが導入済みなら、このリンクを開くとインストール画面が出る)
3. `https://www.tumblr.com/dashboard` を開く

`@updateURL` を埋め込んでいるので、以降は Tampermonkey が新版を自動検出する。自分でビルドする場合は `pnpm build:userscript` で `dist/endless-endless-summer.user.js` を生成。
