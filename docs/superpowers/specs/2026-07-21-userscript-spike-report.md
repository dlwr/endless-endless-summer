...
| — | ネイティブキーボード(注入ポスト) | **成立**(J/K ナビ・L ライク・**Shift+R リブログ**が注入ポストでも動作) |
| E | ライブ pagination 連続性(Stage 5) | **成立**(毎ページ新ランダムポストが append・前進、無限ローディング/重複ループなし) |

> **キーボードの訂正(実装 E2E で判明)**: Tumblr のネイティブなリブログキーは **`Shift+R`** であり **`T` ではない**。spike 中に「T で効いた」とした観察は誤り。`t`=即リブログはユーザー独自スキームで tumblr.com には存在せず、userscript で自前実装する必要がある(follow-up)。GIF 自動再生は Tumblr/OS 設定(視差効果を減らす等)依存で、userscript 無効でも再生されないため本方式とは無関係。
...
### 丸ごと置換の失敗(ページング)と解決(Stage 5)
- 全 `/timeline/dashboard` レスポンスを**固定** donor で丸ごと置換すると、**無限ローディングに陥る**(同一 ID が返り続け append されず、無限スクローラーが再取得を繰り返す)。
- **解決策を Stage 5 で実証**: 毎ページ「ランダムブログ×ランダム `before=`(年均等)」で**異なるユニーク ID のポスト**を返し、`streamGlobalPosition` を全ページ通しで**単調採番**、`_links.next` は**原レスポンスのものを温存**すれば、スクローラーは append し続けて前進する。無限ローディング・重複ループとも発生しなかった。
- 封筒に最低限必要だったのは「毎ページ異なるユニーク ID の要素」+「単調 `streamGlobalPosition`」+「`_links.next` の存在」。`streamSessionId` の自前生成は不要だった(原レスポンスの timeline をそのまま温存で足りた)。
...
- **Tumblr ネイティブのショートカットが注入ポストでもそのまま効く**: `J/K`=ポスト移動、`L`=ライク、**`Shift+R`=リブログ**。→ 2026-07-17 に userscript を却下した理由(キーボード自前実装)は大幅に軽減。ナビ・リブログ・ライクはネイティブ流用でよい。
- **`T` はネイティブのリブログキーではない**(効かない)。ユーザー独自スキームの `t`=即リブログ・`shift+t`=ダイアログ・`r`=リロール・`?`=ヘルプ・`o`=元ポスト新規タブは、いずれも userscript で自前実装が必要(**follow-up**)。
- **注意(実装時のハマり)**: ページ内に追加した要素(トグルボタン等)が**キーボードフォーカスを奪うと Tumblr のショートカット全体が無効化される**(操作要素にフォーカス中と見なされる)。追加要素は `tabindex=-1` + `mousedown` の既定動作抑止でフォーカスを奪わないこと。
- **GIF 自動再生**は Tumblr/OS 設定(視差効果を減らす等)依存。userscript 無効でも再生されないため本方式とは無関係。
